...
# Paths to datasets
data_path = "data/vent_dataset/"
numpy_path = data_path + "numpys/"
model_path = "models/"

#load nifties into numpys
image_vent_ds,masks_vent_ds,img_headers = data.load_nii_ds(data_path,load_cone = False)

#save numpys in relavent dirs
np.save(numpy_path + "image_vent_ds", image_vent_ds)
np.save(numpy_path + "masks_vent_ds", masks_vent_ds)

image_vent_ds = np.load(numpy_path + "image_vent_ds.npy")
masks_vent_ds = np.load(numpy_path + "masks_vent_ds.npy")

print(image_vent_ds.shape)
...
predicted_masks = data.thresh_mask(predicted_masks,thresh = 0.5)

image_nc_ds = np.multiply(image_vent_ds,predicted_masks)
