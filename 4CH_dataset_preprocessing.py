...
# Paths to datasets
path_4CH = "data/4CH_dataset/"
numpy_path_4CH = path_4CH + "numpys/"
model_path = "models/"

##load nifties into numpys
#image_cone_ds,image_vent_ds,masks_cone_ds,masks_vent_ds,img_headers = data.load_nii_ds(path_4CH)
#
##save numpys in relavent dirs
#np.save(numpy_path_4CH + "image_cone_ds", image_cone_ds)
#np.save(numpy_path_4CH + "image_vent_ds", image_vent_ds)
#np.save(numpy_path_4CH + "masks_cone_ds", masks_cone_ds)
#np.save(numpy_path_4CH + "masks_vent_ds", masks_vent_ds)

image_cone_ds = np.load(numpy_path_4CH + "image_cone_ds.npy")
image_vent_ds = np.load(numpy_path_4CH + "image_vent_ds.npy")
masks_cone_ds = np.load(numpy_path_4CH + "masks_cone_ds.npy")
masks_vent_ds = np.load(numpy_path_4CH + "masks_vent_ds.npy")

plt.imshow(image_cone_ds[0,:,:,0])
plt.show()
...
#    mask = data.get_largest_connected_comp(mask)
    ...
    plt.imshow(mask)
    plt.show()
    image_nc_ds[i,:,:,:] = np.multiply(mask,image_vent_ds[i,:,:,:])/255

np.save(numpy_path_4CH + "image_nc_ds", image_nc_ds)
